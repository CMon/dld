...
void nRFCMD_HaltBlinking(u_int32_t code)
{
    u_int32_t i;
    while(1)
    {
...
void nRFCMD_ReadWriteBuffer(const unsigned char *tx_data, unsigned char *rx_data, 
					unsigned int len)
{
    ...
    if(xSemaphoreTake(xnRF_SemaphoreDMA,100)!=pdTRUE)
        nRFCMD_HaltBlinking(2);
} 
